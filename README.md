...
你最终会得到一份**单文件 HTML 报告**：双击就能开、可以发邮件/微信。它是一份**叙事版滑动年报**——整屏翻页，从概览、人格、情绪到待跟进一眼看完，适合分享。不止做统计，里面会有：

- 哪些群和联系人最活跃，最近的聊天节奏、昼夜分布、响应时延
- 哪些私聊有商业机会、哪些对话值得跟进
- 你的高频表达、口癖和说话风格（启发式）
...
这个项目的内核是“一套本地分析引擎 + 单文件报告”（已上线能力见上面的[当前能力](#当前能力)），后续主要是**接入更多数据源、扩展更多画像维度**，所有新增能力都会延续“分析全程零联网、内容不出本机”这条底线。
...
除了直接用 CLI，你也可以把它装成 [Claude Code](https://claude.com/claude-code) 插件，用自然语言驱动整套分析。本仓库本身就是一个 Claude Code 插件市场（marketplace），在 Claude Code 里执行：
...
装好后，直接对 Claude 说"分析我的微信聊天记录""看看最近的客户线索""生成一份微信滑动年报"之类的话，就会触发 `analyzing-wechat-chats` skill，由它来编排导出、特征层、分析和出报告。

> 插件只包含编排用的 skill，不含 CLI 本体。skill 运行时仍需要 `./wechat-insight` CLI——本机还没装的话，它会引导你跑下面的一行安装脚本。
